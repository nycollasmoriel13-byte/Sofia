...
with st.sidebar:
    ...
    menu = st.radio("NavegaÃ§Ã£o", ["ðŸ“Š Dashboard", "ðŸ‘¥ CRM & Leads", "ðŸ“ˆ InteligÃªncia de Produto", "âš™ï¸ ConfiguraÃ§Ãµes"])
...
if menu == "ðŸ“Š Dashboard":
...
elif menu == "ðŸ‘¥ CRM & Leads":
    st.title("ðŸ‘¥ GestÃ£o de Relacionamento")
    
    tab1, tab2 = st.tabs(["ðŸ“‹ Lista de Clientes", "ðŸ‘¤ Perfil Detalhado (IA)"])
    
    with tab1:
        st.subheader("Leads Ativos e Pendentes")
        search = st.text_input("ðŸ” Buscar cliente por nome...", "")
    ...
    with tab2:
        ...
        if selected_client:
            ...
            with p2:
                st.markdown("#### ðŸ§  InteligÃªncia da Sofia (Resumo da IA)")
                st.info(client_data['resumo_ia'])
                
                st.markdown("#### ðŸ“ PrÃ³ximos Passos Sugeridos")
...
elif menu == "ðŸ“ˆ InteligÃªncia de Produto":
    st.title("ðŸ“ˆ O que estamos vendendo?")
